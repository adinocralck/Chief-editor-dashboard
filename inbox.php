...
$a = 0;
$name = "Eric";
$sql = mysqli_query($conn, "SELECT*FROM chat WHERE name='$name' ORDER BY ID ASC");
while ($row = mysqli_fetch_array($sql)) {
    $name = $row['name'];
    $pdate = $row['pdate'];
    $message = $row['message'];
...
<div class="send-mess-wrap">
    <span class="mess-time"><?php echo $pdate;?></span>
...
<form  action="inbox.php" method="POST" class="au-form-icon">

  
  <input name="message"class="au-input au-input--full au-input--h65" type="text" placeholder="Type a message" >
...
<?php
 ...
 if (isset($_POST['send'])) {
    $sql = mysqli_query($conn, "INSERT INTO chat (name, pdate, message) VALUES ('$name', '$date', '$sms')");
  
   headers('location: inbox.php?ok=ok');
 }
?>
</form>
                                                    </div>
...
<?php
 ...
 if (isset($_POST['send'])) {
   ...
   header('location: inbox.php?ok=ok');
 }
?>
</form>
                                                    </div>
